...
from model import UNet_3Plus#UNet_3Plus #U_Net #UNet_3Plus #UNet_2Plus
from utils import *
from metrics import *
from torch.utils.tensorboard import SummaryWriter

from swin_upernet.segmentors.encoder_decoder import EncoderDecoder
from swin_upernet.builder import build_segmentor

norm_cfg = dict(type='BN', requires_grad=True) 
# norm_cfg = dict(type='SyncBN', requires_grad=True)
cfg_model = dict( #EncoderDecoder model (backbone: Swin, decode_head: Uperhead, auxiliary_head: FCNhead)
    type='EncoderDecoder',
    pretrained=None,
    backbone=dict(
    ...
    decode_head=dict(
        ...
        align_corners=False,
        loss_decode=dict(
            type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.0)), # need to change loss?
    auxiliary_head=dict(
        type='FCNHead',
        ...
        concat_input=False,
        dropout_ratio=0.1,
        num_classes=2, # 19 need to change
        norm_cfg=norm_cfg,
        align_corners=False,
        loss_decode=dict(
            type='CrossEntropyLoss', use_sigmoid=False, loss_weight=0.4)), # need to change loss(mae)
...
class Solver():
    ...
    def build_model(self):
        ...
        self.net = build_segmentor(cfg_model)#UNet_3Plus(img_ch=self.img_ch, output_ch=self.output_ch)
        # self.net = UNet_3Plus(img_ch=self.img_ch, output_ch=self.output_ch)
        # model = build_segmentor(cfg_model)

        # Load model from checkpoint
        if self.checkpoint != None:
            ckpt_file = 'best_mae_illum.pt' if '/' not in self.checkpoint else os.path.split(self.checkpoint)[1] #best.pt -> 399.pt
        ...
        if torch.cuda.device_count() > 1:
            # self.net = nn.parallel.DistributedDataParallel(self.net.cuda(),device_ids=[torch.cuda.current_device()],
            # broadcast_buffers=False)
            self.net = nn.DataParallel(self.net)
        #torch.nn.parallel.DistributedDataParallel
        # GPU & optimizer
        self.net.to(self.device)
        self.optimizer = optim.AdamW(list(self.net.parameters()),
                                      self.lr, [self.beta1, self.beta2]) #optim.Adam for Unet #AdamW for swin
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for i, batch in enumerate(self.train_loader):
                ...
                # print("inputs:")
                # print(inputs)#
                # print(inputs.shape)  #torch.Size([8, 3, 256, 256]) = batch_size, rgb, h, w   
                # print("Inference model")  
                pred = self.net.module.encode_decode(inputs) #pred = self.net(inputs)
                # pred = self.net.module.auxiliary_head.forward(pred)
                # print("Pred is")
                # print(pred.shape)
                # print("pred is ")
                # print(pred)
                # print(torch.sum(pred, axis = -1))
                # print("pred:")
                # print(pred)#
                # print(pred.shape)  #torch.Size([8, 2, 256, 256])    = batch_size, output_channel(u,v), h, w   
                ...
                # torch.distributed.destroy_process_group() ##added
            ...
            for i, batch in enumerate(self.valid_loader):
                ...
                pred = self.net.module.encode_decode(inputs)
    ...
    def test(self):
        ...
        f = open(self.result_path+"/output.txt", "w")
        ...
        for i, batch in enumerate(self.test_loader):
            ...
            # error metrics
            # print(inputs.shape)
            # inputs_detach = inputs.detach() ##added 
            inputs_illum = input_rgb / (input_rgb + 1e-8)
            # print(pred.shape)
            # print(pred_detach.shape)
            # ones = torch.ones_like(inputs_detach[:,:1,:,:])
            # inputs_illum = torch.cat([inputs_detach[:,:1,:,:],ones,inputs_detach[:,1:,:,:]],dim=1) #added
            # print("inputs_illum shape")
            # print(inputs_illum.shape)
            # print(gt_illum.shape)
            # print(pred_illum.shape)
            input_MAE_illum = get_MAE(inputs_illum,gt_illum,tensor_type="illumination",mask=batch["mask"]) #added
            ...
            if self.save_result == 'yes':
                ...
                f.write('Input'+batch["place"][0]+'_'+batch["illum_count"][0]+': ' + f'{input_MAE_illum:.4f}' + '\n')    
                f.write(batch["place"][0]+'_'+batch["illum_count"][0]+': ' + f'{MAE_illum:.4f}' + '\n')           
        ...
        print("MAE_illum :", np.mean(test_MAE_illum), np.median(test_MAE_illum), np.max(test_MAE_illum), np.min(test_MAE_illum))
